-----------------------------------
-- func: wanzo
-- desc: Spawns Wanzo-Unzozo for escort quest testing
-----------------------------------
...
commandObj.onTrigger = function(player, npc)
    ...
    local wanzo = npc:getZone():insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            npc:setLocalVar("progress", escortProgress.ENROUTE)
            npc:setSpeed(80)
            npc:pathThrough(route, xi.path.flag.PATROL)
            npc:setAutoAttackEnabled(false)
            npc:setAllegiance(xi.allegiance.PLAYER)
            npc:setIsAggroable(true)
            player:printToPlayer('Initialised', 0, npc:getPacketName())
            player:messageSpecial(7461)
        end,
    })
end
